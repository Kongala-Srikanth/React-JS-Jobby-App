...
class Jobs extends Component {
  ...
  onEmploymentLocation = id => {
    if (id === 'HYDERABAD') {
      this.setState(
        prevState => ({hyderabad: !prevState.hyderabad}),
        this.getJobsData,
      )
    } else if (id === 'CHENNAI') {
      this.setState(
        prevState => ({chennai: !prevState.chennai}),
        this.getJobsData,
      )
    } else if (id === 'BANGALORE') {
      this.setState(
        prevState => ({bangalore: !prevState.bangalore}),
        this.getJobsData,
      )
    } else if (id === 'DELHI') {
      this.setState(prevState => ({delhi: !prevState.delhi}), this.getJobsData)
    } else if (id === 'MUMBAI') {
      this.setState(
        prevState => ({mumbai: !prevState.mumbai}),
        this.getJobsData,
      )
    }
  }
  ...
  locationCheckBoxContainer = () => (
    <>
      ...
      <ul className="checkbox-container">
        {employmentLocations.map(each => (
          <li className="checkbox-each-list" key={each.locationId}>
            <input
              type="checkbox"
              id={each.locationId}
              onClick={() => this.onEmploymentLocation(each.locationId)}
            />
            <label className="label-text" htmlFor={each.locationId}>
              {each.location}
            </label>
          </li>
        ))}
  ...
  jobsContainer = () => {
    const {jobsData, hyderabad, delhi, bangalore, chennai, mumbai} = this.state
    console.log(jobsData)
    const filterJobData = jobsData.filter(each => {
      if (each.location === 'Delhi' && delhi) {
        return each
      }

      if (each.location === 'Hyderabad' && hyderabad) {
        return each
      }

      if (each.location === 'Mumbai' && mumbai) {
        return each
      }

      if (each.location === 'Chennai' && chennai) {
        return each
      }

      if (each.location === 'Bangalore' && bangalore) {
        return each
      }
      ...
      return null // just for error this list is not use
    })
    return filterJobData.map(each => <JobItem key={each.id} jobsData={each} />)
  }
      ...
      <div className="jobs-page-bg-container">
        ...
        <div className="jobs-page-main-container">
          <div className="filter-container">
            ...
            {this.radioContainer()}
            {this.locationCheckBoxContainer()}
          </div>
